...
class ProviderDataFetcher:
    ...
    BASE_PROVIDER_URL = "http://localhost:5000/"
    AUTH_URL = "auth"
    ...
    def hash_token(token):
        return hashlib.sha256(f"{token}/{ProviderDataFetcher.PROVIDERS_URL}".encode('utf-8')).hexdigest()

    def get_auth_token(self):
        auth_url = f"{ProviderDataFetcher.BASE_PROVIDER_URL}{ProviderDataFetcher.AUTH_URL}"
        try:
            response = self.requests_adapter.get(auth_url)
            if response.ok:
                self.auth_token = self.hash_token(response.headers.get('Super-Secure-Token'))
            else:
    ...
    def get_provider_data(self):
        ...
        if self.auth_token:
            try:
                response = self.requests_adapter.get(provider_data_url, headers={"X-Request-Checksum": self.auth_token})
                if response.ok:
                    try:
                        self.data = response.json()
                    except json.decoder.JSONDecodeError:
                        logger.exception("Invalid data for request %s", provider_data_url)
                else:
